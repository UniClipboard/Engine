...
- 本次只做规划，尚未执行 OpenMLS 的跨平台编译和三成员运行验证；该验证已列为实施阶段 0 的硬门槛。
- 当前 Rust 工具链为 1.95.0；桌面、iOS、Android、HarmonyOS 对应的 Rust 目标均已安装，可直接执行阶段 0 的目标构建验证。
- 根工作区显式列出成员；独立验证 crate 需要显式加入工作区，才能由锁文件和全工作区检查约束。
- OpenMLS 0.8.1 的三成员撤销、离线按代次补提交、并发分叉恢复和冷存储恢复均已由独立测试实跑通过。
- 桌面、iOS、Android、HarmonyOS Rust 目标均已重新编译通过；模拟器与真机未执行，必须记为跳过。
