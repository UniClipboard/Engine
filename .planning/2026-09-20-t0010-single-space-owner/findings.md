...
- t-0028 的独立进程证据已经把现象固定在公开接口：自动重试在事件序列“开始、失败、开始、失败、开始、收到真实回复”之后仍保持 `retryable_failure`；需要处理在清除注入后“开始、收到真实回复”也没有完成。
- `events.json` 中的 `finalPhase: healthy` 是旧兼容投影或测试脚本结果，不能代替新的 `space_device_update`；本阶段必须以公开整体字段为唯一断言。
- 当前 Engine E2E 只断言旧维护健康从失败恢复，尚未断言同一真实独立进程中统一整体状态回到 Completed，因此此前测试没有覆盖 t-0028 的精确症状。
- 已确认测试 seam：独立 connectivity host 的公开设备查询为最终断言；dev-only 有序事件仅用于无固定 sleep 地确认真实回复已经到达。
- Engine 真实双端测试新增统一公开状态断言后稳定红灯：历史回复已收到，120 秒内状态仍为 `RetryableFailure`，并带新的绝对重试时间；同一期间日志出现 `membership_group_update` 的 `unavailable`。这说明“历史回复成功”没有覆盖整个空间更新，优先假设是另一项安全更新仍失败，而不是立刻认定历史结果未清除。
- 当前待证伪的三类原因：安全资料投递仍失败；历史成功未覆盖旧结果；成功已持久化但公开查询使用旧快照。下一步读取同一时点的安全更新任务和历史持久结果，并用重启结果区分。
- 与 t-0028 同边界的独立 `uc-connectivity-host` 双进程测试已建立：通过公开 `eligibility` 查询观察统一状态，有序测试事件只等待真实回复。修复前稳定在 30 秒后仍为 `retryable_failure`，带持续后移的重试时间。
- 安全资料单测证明邀请方激活后的持久待发送集合错误地包含本机收件人：构造本机和一台远端时实际留下 2 项，正确结果应只留下远端 1 项。该本机任务无法通过对端网络投递，会把整体状态永久保持为可重试失败。
- 最小修复边界是：协议仍为每个既有正式设备生成同一份可验证资料，但邀请方激活本机已安装的新安全状态时，不把发给本机的副本保存成网络欠账；远端副本和加入方的可靠转发均保留。
- 严格复核后不再让上层额外传入“本机设备”作为第二份事实；Infra 直接从当前已认证安全状态识别本机，并要求它在正式收件人中恰好出现一次。缺失、重复或状态不可读均保持失败关闭。
- 兼容旧资料时只删除收件人为当前本机的普通待投递项；撤销流程、远端收件人、失败分类和历史结果均不改写。旧资料无法证明本机身份时不自动清理，也不会误报完成。
- 查询和维护共用同一份加密欠账。修复后的独立双进程场景中，自动重试、明确解除需处理、失败期间重启和完成后重启都只以公开 `space_device_update` 判定最终结果；内部事件只用于等待真实回复。
