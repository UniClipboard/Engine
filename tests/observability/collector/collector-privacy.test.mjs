...
const SPAN_RULES = [
  ...
  'span.name != span.attributes["uc.operation"]',
  ...
  'span.attributes["uc.flow.id"] != nil and (span.attributes["uc.domain"] != "space_admission" or IsMatch(span.attributes["uc.operation"], "^(space_admission|network_transport)$") == false or span.attributes["uc.role"] != "joiner" or span.kind != SPAN_KIND_CLIENT)',
  'span.trace_state != ""',
  'Len(span.events) != 0',
  'Len(span.links) != 0',
  'span.status.message != ""',
...
const CONSISTENCY_RULES = [
  ...
  '(span.attributes["uc.operation"] == "space_admission" and ((span.attributes["uc.role"] == "joiner" and span.kind != SPAN_KIND_CLIENT) or (span.attributes["uc.role"] == "sponsor" and span.kind != SPAN_KIND_INTERNAL) or IsMatch(span.attributes["uc.role"], "^(joiner|sponsor)$") == false))',
