...
| 旧空间升级 | 只在 `Legacy` 范围运行；`CurrentHistory` 模式永不运行旧升级 |
...
5. 只有明确旧空间模式且当前历史为空时，旧升级运行期才取得 `Legacy` 范围。
...
### Scenario: 明确的旧空间尚无当前成员历史
**Expected behavior:** 只允许旧升级流程使用 `Legacy` 范围；完成当前历史初始化后永久切换为 `CurrentHistory`。
**Implementation:** 模式来自安全状态，不由历史读取失败或空表猜测。
...
6. 明确旧模式且历史为空：只生成 `Legacy` 范围；切换当前历史后不再读取旧范围。
...
3. 低于 `1.1` 的明确旧空间仍能完成原地升级；当前历史设备不运行旧升级。
...
旧空间需要在当前历史建立前读取旧成员资料。该例外通过显式模式进入，增加一条受控分支；若简单规定
“永远只读历史”，会破坏仍受支持的原地升级。模式一旦切到当前历史不得回退。
