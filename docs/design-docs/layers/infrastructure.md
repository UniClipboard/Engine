...
* setup 流程推进
* 配对等领域状态机的状态定义、合法转换与阶段推进
* “先 A 再 B 再 C”的 use case orchestration
* 用户输入驱动的业务状态流转

状态定义与合法转换属于 `uc-core`（见 [Core 设计规范 §5](core.md#5-状态机)），推进顺序属于 `uc-application`。
Infra 不得直接写入 Core 状态字段或自行推进阶段，只能保存 Core 返回的新状态。
...
infra 可以执行校验，但不能拥有业务真相。跨多条记录的判定（例如“同一时间最多一个未结准入”）由 Core 以纯函数给出，
Infra 仓储只负责在同一事务中读取相关记录、调用该判定并执行结论，见 [Core 设计规范 §5.5](core.md#55-跨对象不变量)。
