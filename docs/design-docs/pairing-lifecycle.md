...
| 工作 | 当前时间模型 | 到期或失败后 |
| --- | --- | --- |
| 配对尝试 | `AdmissionAttemptTimeline`：开始时间加 300 秒；解析、认证、重连和重启不续期 | 可终止阶段先本机保存终态，再处理网络；已正式完成不回滚 |
| Joiner 放弃通知 | 沿用原 attempt 截止时间，不另加五分钟 | 无本机切换待办时，到期、缺少期限或通知协议不是当前版本可结束待发送项，保留围栏 |
| 已非当前成员对端的受限通知 | 移除或决策提交时不计时（`updated_at_ms` 写为 0）；首次维护处理该投递时保存起点，从起点起 300 秒 | 到期先核对观察到的整条记录未变化，再条件删除；不宣称对端确认 |
| 设备组密钥更新 | store 的持久 `next_attempt_at_ms` 与失败分类 | 离线/传输失败延期，稳定拒绝需处理；没有共用配对五分钟终止规则 |
