...
| 失败记录只有 `reason="unknown"` | 上游转换把错误字符串化或丢弃，分类器沿 source chain 找不到具体类型；补日志字段无济于事。见[错误来源与日志字段](#错误来源与日志字段)。 | 用真实下层错误（如 SQLite `BUSY`、IO 权限拒绝）走完整转换路径，断言记录中的固定分类值。 |
...
- **统一写法。** 记录点写固定的 `error_kind` 字面量说明失败的动作类别，再用
  `uc_observability_contract::error_source::io_error_kind(..)` 从来源链提取 `io_error_kind`（`ErrorKind` 变体名，
  链上没有 IO 错误时字段省略）；`anyhow::Error` 与装箱错误以 `.as_ref()` 传入。需要识别其他下层类型时用同模块的
  `find_source::<T>(..)` 在所属层内扩展，不回退到正文。无字段的固定枚举可直接以其变体名作为 `error_kind`。
...
现有代码中的字符串化与日志正文清单见[错误来源保留执行计划](../exec-plans/active/2026-09-24-error-source-preservation.md)。
...
入站对端拒绝另以本地运行诊断 `peer.inbound.rejected` 记录固定协议和原因。展开字段仅有 `event.name`、
`direction=inbound`、`protocol`、`uc.outcome=rejected`、`error.phase` 和 `error.reason`；不含对端标识、
公钥、指纹或地址，不改变拨号侧收到的拒绝结果。
