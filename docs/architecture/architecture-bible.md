...
- 2026-09-16：自写回显归因从一次性消费改为按单次写入的回显集。远程推送的 next-change fallback 拥有两次同内容类别吸收预算，内容命中消耗配对 fallback 的一次预算而非删除，且首次回显后剩余预算只保留 5 秒；本地恢复保持一次性语义。该修改补齐“一次程序化写入产生多个系统事件”时的归因，避免第二个事件被误判为本地捕获并回传对端形成 A↔B 图片回环。该提交重基线到当前 main；同一分支中针对旧 `WorkspaceConvergence`、`space_setup` 与 `DurableAdmissionSpaceTransition` 的其余修复随本次重基线放弃，其职责已由单一 Space 准入重写、准入自动恢复与准入到期取代。不改变分层、公开接口或持久化格式。
