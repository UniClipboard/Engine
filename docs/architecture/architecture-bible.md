...
每次新 Join 从 Engine 接受操作起保存唯一五分钟期限。解析邀请、等待认证和 Candidate 阶段可以在本机到期、取消或被新意图替换；Prepared 及以后也先保存本机终止结果，不等待远端在线或先恢复成功。结果与 current 指针原子保存后释放加入入口；远端决定未知时保留尝试责任，决定已知时固定原 Space、成员实例和 Add。时间线、终止原因、准确放弃消息和清理责任继续由现有准入仓储逐条加密；旧记录没有期限，不补造开始时间。唯一成员维护运行期在启动、恢复、状态变化和对端上线时重试同一放弃消息。接收方先保存终止围栏与准确回复，再把已知或从当前历史核实出的原成员交给成员移除流程；网络重试、重复通知和旧阶段消息都不能重新选择同设备的新成员实例。跨 Space 已激活状态的本机退出仍由后续切换切片完成。
