...
- 2026-09-17：新增[已知设备联系驱动的成员恢复与地址更新计划](../exec-plans/active/2026-09-17-known-peer-contact-recovery.md)，并完成前两切片。经过传输身份验证、能映射到当前成员但尚未准入的入站联系会唤醒该目标的成员确认，不等待持久退避到期；联系仍不代表成员资格、准入或在线。完整成员历史交换成功后，只把本次正在使用的 relay 与固定 NodeId 写回既有加密地址仓储，不保存动态直连地址，也不以无 relay 的观察覆盖旧记录。成员维护继续负责合并、串行与周期兜底；公开接口、设备间协议和持久格式未改变。
