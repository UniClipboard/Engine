...
- 2026-09-21：资料口令恢复完成后若准入仓库仍无法验证，Engine 在同一进程直接转入既有受限恢复状态，不启动成员维护、同步或其他后台能力；仓储内部错误继续保留原始来源，对外仍只报告稳定类别、阶段和恢复方向。公开接口、持久格式和流程所有权未改变。

- 2026-09-21：在本线程的隔离资料中复测旧版升级后的准入状态读取：合成的仓库元数据损坏可重复产生升级跳过、读取失败和启动受阻的顺序；这不是用户现场根因认定。本次只增加调查脚本和证据，未改变架构、生产行为、接口或持久格式。
- 2026-09-21：后续修复在 Space Admission 恢复负责人处集中判断准入读取失败的稳定类别和恢复方向；Engine 复用资料恢复公开状态提供最小受限会话，阻止加入、成员维护和同步。没有改动持久格式，也没有实现自动修复；完整备份、权威事实核验及原子替换仍是将来执行恢复的前置条件。
- 2026-09-20：隔离资料副本的 Engine 宿主验收补齐空目录复制，以便使用旧版测试归档核验启动、历史读取与再次启动；仅修改测试准备，架构与运行期行为未改变。
- 2026-09-20：旧版已激活 V3 资料恢复时，只在认证清单与升级记录代际一致的后提升校验中，允许旧版尚未创建的四张另一库归属表缺失；表存在时仍须为空，数据库完整性、外键、其他必需表及提升前校验不变。随后由既有常规迁移补建新表；升级流程负责人、公开接口及持久格式未改变。
...
- 2026-09-21：准入仓库把事务内读取与普通读取统一收口到同一解码和迁移实现，避免写入流程在损坏资料上嵌套开启独占事务并丢失原失败类别；公开提交入口补充读取后、提交前资料损坏验收，确认仍返回同一稳定恢复分类并保留内部来源链。公开接口、持久格式和安全门禁均未改变。
- 2026-09-21：正式交付前增加显式启用的隔离损坏资料宿主验收，连续三次只在临时副本中确认稳定受限恢复及加入、发送、成员维护和同步查询拒绝；正式产品行为、公开接口、持久格式和流程负责人均未改变。
- 2026-09-21：旧 V3 资料升级兼容与准入仓储受限恢复合并验收。升级前校验只允许缺少后来新增且当时无权威数据的四张表，迁移后仍要求完整结构；公开 Engine 回归同时证明健康资料保留历史与成员状态并正常启动，而无法认证的准入元数据即使处于同一缺表形态仍连续进入稳定受限恢复，禁止成员维护、同步和资料写入。公开接口、持久格式、身份校验和流程负责人均未改变。
...
- 2026-09-21：准入读取恢复补齐公开失败分类的分支验证，并在覆盖构建中排除必须由显式本地隔离资料驱动的忽略测试；测试仍只通过准入仓库公开读取结果和 Engine 公开宿主边界验收。生产流程、公开接口、持久格式和安全门禁均未改变。
