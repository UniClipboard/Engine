...
| 2026-09-08 | HarmonyOS 发布构建环境隔离 | 自托管发布机的用户级、工作区或残留临时 Cargo 配置可能把仓库依赖替换为开发机本地路径；HarmonyOS 发布步骤移走用户和工作区配置，并为每次运行隔离 HOME、创建新的临时 Cargo 主目录，确保锁定构建只读取发布提交中的依赖来源。 |
