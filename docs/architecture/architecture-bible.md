...
| 2026-09-11 | 桌面宿主诊断计划范围 | 规格 041 补齐桌面 daemon、GUI、CLI 的接入与离线导出责任，分别设置手机、桌面和联合验收阶段；桌面 Engine 采集由 daemon 唯一拥有。此项为计划范围，不代表桌面新增采集接口已经交付。 |
| 2026-09-11 | 可导出连接诊断 | [规格 041](../exec-plans/active/041-exportable-connection-diagnostics.md)的进程运行时拥有限时采集、随机本地关联、来源覆盖和有界文件输出；Infra 观察实际连接、发现、存储与路径，节点用弱句柄拥有观察任务收尾。成员更新保留真实错误来源，固定详情只附加到本地完成记录。Engine 与平台绑定只暴露诊断控制和封闭宿主事实；产品接入与两端真机仍待验收。 |
