...
邀请签发对目录普通传输失败和服务暂时不可用保持局域网降级；目录明确拒绝或成功响应无效时不得生成另一短码绕过，因为目录可能已经接受了对应完整邀请。无可发布地址、局域网发布失败、目录传输失败、目录拒绝和目录响应无效分别通过稳定公开错误表达；下层来源只保留在内部错误链中，公开错误与日志不包含地址、身份、邀请、令牌或响应正文。实现与验收记录见 `docs/exec-plans/completed/2026-09-20-windows-invitation-failure-classification.md`。

产品分析以 `pairing_started` 记录进入 Engine 的邀请生成尝试，以 `pairing_invitation_issued` 和 `pairing_invitation_failed` 记录互斥终态。成功只在邀请已经完成本地保存后记录；失败由 Engine 完成公开错误转换后记录稳定编号、公开类别和是否可重试，不上传内部来源或业务负载。表示加入方失败的 `pairing_failed` 不参与邀请生成结局统计。
...
- 2026-09-20：Windows 首次邀请失败补齐分阶段注入和稳定公开分类；普通目录传输失败继续降级为局域网邀请，目录拒绝和无效成功响应不再静默吞掉。既有现场记录只能排除“完全没有本地地址”，不能确定局域网发布或目录响应哪一项真实失败，等待 Desktop `t-0016` Windows 实机复验。
- 2026-09-20：邀请生成分析补齐独立失败终态，使用公开错误编号、类别和可重试性；成功记录移到本地收尾完成之后。开始边界不变，生成成功与失败互斥，加入配对失败事件保持原语义。
...
- 2026-09-22：规格 034 完成首个最小多节点切片：test-only 有向成员网络只注册节点、施加分区、限制 frame 并路由 typed 成员历史消息，目标节点继续执行真实 Application endpoint；真实 Engine nightly/手工入口复用现有 connectivity host、relay、network namespace 和证据脚本，并分别记录编译、场景清理及总耗时。该切片不实现完整配对，不代表五类路线覆盖完成，也不改变生产架构、公开接口、协议或持久格式。
...
- 2026-09-22：Engine 测试长期路线统一收敛到测试架构文档：日常与 PR 使用快速确定性线，nightly 和手工入口使用真实 Engine 多进程、独立资料、真实存储与相应真实网络；首批覆盖配对、文字和文件传输、断线重连、重启恢复及旧资料升级。testkit 只承担测试生命周期、故障施加、等待、资源和诊断，不复制业务状态机；本次仅更新测试路线与计划关系，不改变生产架构、公开接口、协议或持久格式。
