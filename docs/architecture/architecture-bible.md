...
重复的 `Applied` 表示等待设备列表或安全代次已经向前变化。确认永久丢失前，成员移除负责人必须以当前安全组为准逐一核对等待设备：已经不在组里的设备只收尾旧等待记录，不重复移除；仍在组里的设备才追加下一轮安全更新并通知留下的设备。无法读取当前安全组或无法把记录与可信成员对应时，流程进入 `RecoveryRequired`，停止自动处理并只允许用仍可信、仍在使用的设备重建安全组；永久丢失设备不得重新加入。永久丢失恢复继续使用同一流程编号，并可追加多轮安全更新；后一轮不能越过前一轮。`Complete` 和 `RecoveryRequired` 是终态，后者不能用永久丢失入口绕过。
