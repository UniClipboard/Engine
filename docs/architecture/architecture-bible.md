...
Engine 活跃期间，`SessionSupervisor` 在同一个运行状态中共同持有唯一长期网络和当前 Space 会话，禁止独立更新二者形成“有会话但无网络”的状态。普通 Space 切换与设备关系重置只封口并排空旧 Space 的请求，停止旧 Application，再在同一个网络入口上一次发布新 Space 的完整能力；旧能力不会重新开放。会话安装失败而网络仍存活时，既有切换 watcher 从当前权威状态重试安装。暂停、最终关闭和明确请求的网络恢复才完整关闭长期网络。
