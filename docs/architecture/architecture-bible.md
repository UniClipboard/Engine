...
- 2026-09-13：生产启动失败改用最终关闭的同一安全回收规则；网络恢复与会话侧资源使用者同时收到停止通知，只有全部成功结束才交接本地安全资源，原始启动失败继续作为主原因。资料重置的窄入口不变。
- 2026-09-13：最终关闭并行通知进程任务与会话侧工作停止；活动文件传输仍在会话结束后永久关闭，本地安全资源仍等待三类资源使用者全部成功结束后才交接。公开关闭入口与所有权边界不变。
- 2026-09-13：最终关闭同时通知进程级网络恢复与会话侧资源使用者停止；本地安全资源仍等待网络恢复、会话、活动传输和进程任务全部成功结束后才交接，资料重置继续只处理其实际拥有的依赖。公开关闭入口不变。
