...
Engine 活跃期间，`SessionSupervisor` 分别持有唯一长期网络和当前 Space 会话。普通 Space 切换与设备关系重置只封口并排空旧 Space 的请求，停止旧 Application，再在同一个网络入口上一次发布新 Space 的完整能力；旧能力不会重新开放。暂停、最终关闭和明确请求的网络恢复才完整关闭长期网络。
