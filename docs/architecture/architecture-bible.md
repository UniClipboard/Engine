...
旧升级备份记录的保护材料属于同一恢复清单。已有恢复文件打开时，仍留在系统安全存储中的原值先合并进 userdata 密文并复读验证，随后才进入可重试清理；两处都缺失时保留升级安全门槛并返回稳定的不可重试结果。口令恢复后的完整运行期若启动或解锁失败，恢复状态进入 `Failed`，同一进程不得再次提交并明确 `restart_required`，由进程重启重新创建一次性宿主能力。暂停或关闭完整运行期后清除内存中的恢复密钥，恢复运行时从持久材料重新打开。空间创建、加入、解锁或重置已经提交后，若恢复文件刷新失败，原动作仍返回成功，并通过恢复状态和事件报告刷新失败。

改口令复用既有耐久记录：先让恢复文件同时接受旧、新两份包装，再安装新口令材料，复读确认后只保留新包装，最后清除记录。任一落盘边界中断后，旧口令或新口令至少有一条路径能重新打开原资料并继续同一事务。永久缺失的旧资料在启动检查时直接进入不可提交的部分恢复终态，影响范围按历史、控制资料和设备身份分别报告，不写入新的自动解锁材料。磁盘替换在 Unix 同步目录，在 Windows 使用写穿替换；持久资料和加入请求中的口令派生参数先执行同一组单项及总工作量上限检查，再进入密码计算。
