...
- 一个空间同时只允许一项未完成的成员移除；应用层成员移除流程统一负责本机生效、等待、确认、重试、重启恢复、用户授权的永久丢失恢复和最终完成。
- 同一目标重复移除返回原状态；不同目标返回稳定冲突。产品端只发起、查询当前状态和提交一次明确授权的永久丢失选择，不编排内部步骤。
- 永久丢失恢复只能排除当前等待设备，并从已生效状态向前追加新安全代次；旧代次不得恢复，其他保留设备必须按顺序补齐所有缺失更新。
- 当前移除、等待设备、确认结果和多轮恢复记录只保存在加密负载中；等待数量由等待设备列表计算，不形成第二份事实。
...
第一次移除在本机推进安全代次后进入 `Applied`，等待设备逐一确认后进入 `Complete`；无法自动收敛的损坏或分叉进入 `RecoveryRequired`。每次状态变化都通过统一事件发送完整状态，产品端错过事件后直接查询当前移除。

普通离线、超时、网络失败、重启和自动重试只会继续等待，不能自动排除设备。用户明确确认某个等待设备永久丢失后，应用层在同一流程中追加新代次和新发送队列，再继续等待其余保留设备确认。被确认丢失的设备不得取得新状态。
...
```text
Recovering -> Applied -> Complete
     |           |
     +-----------+-> RecoveryRequired
```

`Recovering` 表示尚未收束为 `Applied` 或其他终态的成员移除，涵盖 `Prepared`、`Staged` 和 `Activated`，由应用层统一检查和收束，不能伪装为普通等待。重复的 `Applied` 表示等待设备列表或安全代次已经向前变化。确认永久丢失前，成员移除负责人必须以当前安全组为准逐一核对等待设备：已经不在组里的设备只收尾旧等待记录，不重复移除；仍在组里的设备才追加下一轮安全更新并通知留下的设备。无法读取当前安全组、无法读取当前安全状态，或无法确认证明新的准备已持久化时，流程进入 `RecoveryRequired`，停止自动处理并只允许用仍可信、仍在使用的设备重建安全组；永久丢失设备不得重新加入。永久丢失恢复继续使用同一流程编号，并可追加多轮安全更新；后一轮不能越过前一轮。`Complete` 和 `RecoveryRequired` 是终态，后者不能用永久丢失入口绕过。
...
- iOS、Android 和 HarmonyOS 必须为成员移除公开相同的完整状态、稳定错误、当前查询、永久丢失继续入口和变化事件。
