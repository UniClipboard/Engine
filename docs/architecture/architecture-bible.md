...
每次新 Join 从 Engine 接受操作起保存唯一五分钟期限。解析邀请、等待认证和 Candidate 阶段可以在本机到期、取消或被新意图替换；结果先与 current 指针原子保存，再释放加入入口。时间线和终止原因使用独立 Core 记录版本，并继续由现有准入仓储整体加密；旧记录没有期限，不补造开始时间。唯一成员维护运行期按最近截止时间准确唤醒 Recovery，启动和恢复均先保存到期结果再尝试网络。Prepared 及以后可能已导致远端正式决定，仍等待后续精确撤销能力，不按早期规则直接结束。
...
- 当前 S1 中，`Prepared` 后不能由新的 JoinSpace 取代；`Commit` 后取消返回已太晚并继续恢复同一次加入。ADR-026 规划的决定后终止与精确撤销尚未接入。
...
- 2026-09-13：[配对到期、本机终止与新意图替换规格](../exec-plans/active/2026-09-13-admission-expiry-and-replacement.md)已回写六项用户决定并细化 S0–S8 实现切片；[ADR-026](../design-docs/decisions/026-bounded-admission-lifecycle.md)取代“Prepared 后只能恢复成功”的旧规则。S0 已建立固定五分钟尝试与精确成员绑定的验证契约，并增加本机终止、邀请方确认状态及两个移动出口的完整映射；这些展示状态独立于成员同步资格。自动到期、真实状态读取、撤销、网络接入与跨空间清理仍待 S1–S8 实施，不能据此宣称新流程已经可用。
...
- 2026-09-14：新版加入在认证后保存双方共同认证的尝试摘要。Prepared 及之后的加入都允许本机因取消、原期限到期或新意图替换而立即结束，不等待旧设备在线；Prepared 保存远端提交未知的清理责任，已收到 Commit 的阶段保存原空间、成员实例和 Add 的精确清理目标。新增责任使用下一版密文记录，上一版记录布局保持可读。结束记录与当前加入槽位仍由原准入仓储提交，并发迟到回复不能覆盖结束结果。放弃事实投递和跨空间最终隔离仍分别由后续切片完成。
