...
### 2026-09-21 重新浮出与搜索排序时间

条目表中的活跃时间是唯一事实来源，搜索索引保存的副本只用于浏览与搜索排序。此前本地重复复制、入站重新激活和恢复只推进条目表，索引副本停留在首次索引时的值，列表因此不随重新浮出而变化。现在 `ClipboardAssembly` 在入口用 `SearchMirroredTouch` 包装 touch 能力，任何成功的 touch 都通过 `SearchIndexPort::set_entry_active_time` 把同一个值写入索引，并在重建进行中同步到临时文档表。该写入只更新已有文档的活跃时间，不重建词项、标签或渲染负载，也不新增持久字段；镜像失败不回滚条目表，由下一次重建校正。
