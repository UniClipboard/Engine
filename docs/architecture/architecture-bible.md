...
- `CompleteAck` 是邀请方新增正式成员的唯一提交点；此前的握手、候选和准备结果都不授予成员资格。
...
- Sponsor 处理 `Applied` 时只保存加密待确认资料并返回 `Complete`，不写正式成员历史或安全通信成员集合。验证 `CompleteAck` 是正式成员的唯一提交点；之后返回 `Settled`。确认请求或回复丢失时重放同一条消息并幂等收敛。
- Joiner 完成本机 Space 准备后、收到 `Settled` 前公开状态为“处理中”；只有保存 `Settled` 后才公开为正式加入。处理中状态沿用 Engine 持久保存的同一加入编号，宿主重启后通过查询恢复，不保存第二份流程事实。邀请方把等待最终确认的设备单独公开为待确认项，不把它放入普通设备列表；其他设备看不到该候选。
- 候选安全终结后只保留最小失败投影，不补造已经清理的设备资料；失败项不进入正式设备列表，也不阻止已有设备查询或下一次安全配对。无法证明是否提交的旧记录公开固定原因和处理方向，不自动删除。
- 准入协议 V4 承载最终确认后提交、可靠失败收尾和安全失败分类。旧协议与 V4 混用在初始认证阶段明确要求升级，不消费邀请凭据，也不产生候选或正式成员。
- Completion Helper 只能恢复已经提交的同一加入，不能创建成员事实。
- 同一设备重新加入时，签名历史保留旧成员实例作为过去事实；新实例在激活回执验证后接替旧实例成为唯一当前成员，并在同一次安全组变更中替换该设备遗留的全部旧实例。未完成加入不提前撤销旧实例，不能通过临时去重、任选凭据或删除历史绕过冲突。
- JoinRequest 声明的传输身份必须与已认证连接一致。目标历史或控制资料存在不可恢复冲突时保存明确拒绝结果，重启后仍可查询，不继续表现为正常加入中。
...
- 2026-09-22：PR 检查单独运行 `uc-engine/dev-tools` 的单元与端到端测试，并在正式代码覆盖率中排除只供测试进程使用的 `uc-engine/src/dev`；测试控制能力继续接受行为验证，但不再与产品覆盖率混算。正式产品特性、公开接口、运行行为和发布构建均未改变。
- 2026-09-21：配对失败收敛的公开结果补齐稳定原因、处理方向和可选重试时间；已安全终结的入站候选仍以失败尝试单独可见，但不伪造已经清理的设备资料。成员移除若只遇到并发状态变化现明确返回可重试，不再误报为资料损坏。
- 2026-09-21：配对失败诊断补充了两条邀请方待最终确认记录的持久回归：单条记录可正常查询，第二条出现后当前公开查询会整体失败，重启后记录仍保留且没有可执行的终结恢复。测试同时确认这些记录尚未进入正式成员历史；本轮只固定现有缺陷和证据边界，不改变协议、公开接口、持久格式或产品行为。
- 2026-09-21：配对失败与进程退出改为双端有界收尾。加入方稳定失败保存可重发终态，邀请方对可证明尚未正式提交的候选在共同期限后独立终结；最终确认仍是正式成员与安全资料的唯一提交点，提交后只重放原成功结果。旧记录按未提交、已提交和证据不足分别自动收尾、保留成功或进入 `NeedsAttention`，未安全收尾前拒绝叠加新尝试。公开查询分别列出多个候选，不再影响既有正式设备；安全失败只公开固定类别，不记录身份或原始资料。协议提升到 V4，旧端在正式写入前稳定停止。
- 2026-09-21：复核 Desktop t-0028 的三设备重启场景：正式成员历史没有因重启被删除，首个断点是离线旧成员重启后与仍在线新成员无法重建已认证联系，历史交换因此无法开始。固定端口对照和两种地址刷新试验都未使原场景通过，相关试验代码已撤回；本次只记录网络恢复与安全历史引导的后续边界，不改变空间负责人、协议、持久格式或正式行为。
- 2026-09-21：普通成员历史交换的稳定拒绝现在写入既有成员维护进度，设备信任查询可持续返回明确原因和处理方向，后续真实交换成功后回到健康；未新增持久字段或第二份状态。既有 `dev-tools` 独立进程边界同时支持可计数的暂时失败与稳定拒绝、停止继续注入及有序交换事件，正式构建没有入口，事件不包含成员标识或用户资料。
- 2026-09-21：为 Desktop t-0028 的独立进程验收扩展既有 `dev-tools` 测试边界：指定测试进程可让第一次最终确认连接暂时失败，并通过进程私有管道等待、查询最终确认重试、普通成员更新与历史同步的有序事件。正式 Engine 操作、协议、持久格式和运行负责人均未改变；测试故障仍使用生产的持久重试时间与真实网络装配。

- 2026-09-20：新增[单一空间工作负责人实施规格](../exec-plans/active/2026-09-20-single-space-work-owner.md)，计划从同一份持久配对记录派生工作资格：配对未终结时只恢复配对、入站历史不写入，正式提交后才运行普通成员维护；本次仅落盘规格，现有生产调度和协议行为尚未改变。
...
- 2026-09-19：成员正式加入的唯一提交点从 `Applied` 后移动到 Sponsor 验证 `CompleteAck` 后；此前只保存加密待确认资料，不更新正式历史、安全通信成员集合或第三方视图。确认请求、确认回复、双方重启和重复消息通过同一持久状态幂等收敛；加入方等待 `Settled` 时公开为处理中并跨重启保留同一加入编号，邀请方单独公开待确认候选且不混入正式设备列表。协议升级到 V3，V2 混用在初始认证阶段明确拒绝。公开加入结果新增“处理中”，准入记录格式不变。
...
- 2026-09-20：新增[每个 Space 的唯一工作负责人计划](../exec-plans/active/2026-09-20-single-space-work-owner.md)，规定未终结配对只运行配对恢复，普通成员效果、更新、历史同步与入站成员历史均无运行资格；运行模式从同一加密准入记录派生，不新增第二份持久开关。最终确认继续作为正式成员唯一提交点，历史收敛属于终态后的普通维护。本记录只固定待实施规格，当前运行行为、公开接口和持久格式尚未改变。
- 2026-09-20：开始实施单一空间工作负责人：后台维护与认证入站历史现在从同一加密准入记录派生资格，并与本地/入站配对动作共享串行许可；未终结配对只推进配对，入站成员历史在读取账本前返回可重试 BUSY。最终确认仍是正式成员唯一提交点，持久格式未改变；统一唤醒与对外维护健康状态仍在后续切片收口。
- 2026-09-20：单一空间工作负责人完成运行收口：设备上线、联系和显式变化只提交可合并的通用唤醒，普通成员维护不再接收设备身份或绕过持久退避。配对暂时失败推进原加密准入记录内既有重试进度，过早唤醒不联网；普通维护开始前在共享许可内重新读取权威配对状态，配对网络等待仍可被本机取消或替换打断。设备信任查询独立报告成员维护的健康、下次重试时间、稳定原因和恢复动作；没有新增持久格式或第二份运行模式。
- 2026-09-20：单一空间工作负责人完成交付检查。仓库架构门禁现在验证准入负责人提供跨配对、普通维护和入站历史共用的工作许可，而不再要求只保护准入内部的独占锁；最终确认前普通维护和入站历史均无运行资格，完成后恢复正式成员维护。严格复核、全仓编译、格式、架构、隐私及三设备自动验收已执行，实体设备仍待单独授权。
...
- 2026-09-19：正常成员移除以用户确认的设备为安全撤销目标；若旧版本留下同设备多个群组身份，一次群组提交撤销该设备的全部旧身份，不选择凭据、不清理其他设备，也不依据离线状态自动处理。成员效果尚未激活时，设备查询返回处理中而不是已移除；完成后重启恢复与普通邀请继续使用原流程。Application 仍负责移除与效果恢复，Infra 负责单次群组变更；公开接口、持久格式和设备间协议未改变。
- 2026-09-19：邀请资格继续只依赖已验证成员事实，不依赖在线状态或远端是否已取得最新历史；关系一致但同步暂缓不阻止新邀请，活动成员关系未知、待本机决定、分歧、无效或需升级时仍关闭准入。Application 仍负责一次性给出准入结果，公开接口与持久格式未改变。
- 2026-09-19：准入记录 V2 读取旧状态内容时直接复用同一份领域转换，不再重建完整 V1 记录并递归读取；移动端恢复线程在保存终态时不再同时承载两次大型记录解码。准入流程负责人、密文仓储、持久格式和公开接口均未改变。
- 2026-09-19：同一设备使用新成员凭据重新加入既有 Space 时，旧实例继续保留为历史事实，新实例只在激活回执验证后接替当前资格；安全组在一次变更中替换该设备遗留的全部旧实例，已认证连接核对申请中的传输身份。无法自动恢复的激活冲突保存为明确拒绝并可跨重启查询，不再无限显示加入中。公开产品动作和持久格式不变。
...
- 2026-09-21：设备信任查询对外新增唯一的“空间设备状态更新”结果，统一覆盖待生效设备变化、设备关系、安全资料更新和历史确认；只有全部达到可正常同步条件才报告完成。旧维护健康字段只从该结果派生供 Desktop/Mobile 迁移，不是第二份事实。内部子步骤仅进入本地诊断，以固定阶段、错误链、调用路径和 trace/span 上下文定位，不扩大敏感记录或 Engine 流程编排责任。
- 2026-09-21：邀请方正式接纳新设备时，安全资料仍为全部正式设备生成一致的协议交付，但可靠网络欠账只保存远端收件人；本机已原子安装的副本不再重复网络投递。旧加密资料中的本机冗余欠账由当前已认证安全状态识别并幂等收敛，无法证明本机或仍有远端欠账时保持未完成。统一“空间设备状态更新”继续是客户端唯一完成事实，公开接口、设备间格式和正式成员提交点不变。
