...
应用层的 `NetworkRecoveryFacade` 是被明确请求的网络会话恢复的唯一流程负责人。它只向 Engine 请求“重建当前网络会话”这一完整动作，不持有 Endpoint、Router 或第三方网络错误。Engine 的 `SessionSupervisor` 是该动作的唯一执行者，负责关闭操作门、等待或取消旧操作、停止完整旧会话、终结活动传输、创建并安装完整新会话；构建失败时保持操作门关闭。
...
- 2026-09-13：隔离网络验收改为逐条安装防火墙规则，避免测试机拒绝从标准输入读取规则文件；失败时保留首行系统原因并隐藏本次运行标识。断网方式、验收顺序和证据结构均未改变。
