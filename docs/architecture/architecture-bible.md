...
改口令复用既有耐久记录：先让恢复文件同时接受旧、新两份包装，再安装新口令材料，复读确认后只保留新包装，最后清除记录。任一落盘边界中断后，旧口令或新口令至少有一条路径能重新打开原资料并继续同一事务。永久缺失的旧资料在启动检查时直接进入不可提交的部分恢复终态，影响范围按历史、控制资料和设备身份分别报告，不写入新的自动解锁材料。磁盘替换在 Unix 同步目录，在 Windows 使用写穿替换；持久资料中的口令派生参数先执行统一资源上限检查，再进入密码计算。
