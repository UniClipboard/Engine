...
| 2026-09-01 | 配对性能子阶段观测 | `admission.performance` 进一步拆分 Joiner/Sponsor 状态 load/commit、Candidate/Applied/Activation 准备与双方激活执行；无待处理准入的空恢复轮次不产生日志。所有阶段名固定且脱敏，本轮只增加可观测性，不改变协议状态机或持久化边界。 |
