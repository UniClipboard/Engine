...
### 2026-09-21 查询词不再强制整词标签

索引把整段正文作为一个片段分词，正文中的复合词（如 shell 命令里的 `gitlab-runner`、`docker-compose.yml`）只留下各组成部分，整词标签只在该词独占一个片段时存在。查询侧此前对每个查询词同样生成整词标签，并在 `AND` 语义下要求它命中，导致这类词按原样输入时永远查不到。现在分词器区分索引与查询两种模式：查询词只要求组成部分，仅当没有任何组成部分留存时回退到整词。只要整词被索引，其组成部分也一定被索引，因此原本能命中的条目不受影响。改动只在查询侧，不改变已写入的索引内容、`index_version` 或持久格式。
