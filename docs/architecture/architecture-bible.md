...
`ProfileStorageUpgrade` 是该软件升级的唯一 Infra 协调边界；未来 Engine 只调用一次 `ensure_v3()`，不能编排逐表步骤。模块拥有 profile 级进程内串行化、跨进程非阻塞租约和使用 profile 稳定密钥 AEAD 保存的恢复 journal。Journal 显式绑定 V2 source manifest、数据库 revision 及唯一目标 profile data/control generation；`TargetStaged` 前通过 SQLite 一致性快照原子创建两个目标数据库，恢复同时验证 target digest 与 source revision，升级期间出现旧路径写入必须失败关闭。当前两个 target 仍是同一 source snapshot，只建立崩溃恢复与物理 generation 所有权；表归属拆分、V3 payload 转换、production 验证和 manifest promotion 尚未完成，不能把双份 snapshot 接入运行期。
