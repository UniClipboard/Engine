...
- 2026-09-17：Iroh 网络节点由独立关闭负责人持续持有协议与路由清理；宿主期限到达或等待者离开只结束本次等待，不遗弃真实清理。关闭会尝试所有协议并保留多项失败，只有任务实际结束后才释放所有权；公开网络接口和连接策略不变。

- 2026-09-17：本地后台持久工作在暂停时拒绝新动作并排空已开始的数据库与内容物化操作，旧计时批次在恢复后不会补跑；进程任务到达期限后等待取消任务实际析构再报告结束。公开业务入口、持久格式和密文边界不变。
...
- 2026-09-17：文件接收由文件传输负责人统一封闭新会话、取消全部已登记会话并等待结束；超时清理将停止置于定时工作之前，停止信号发送方退出同样视为停止。单个会话取消失败不会遗弃其余收尾，首个失败保留来源；公开入口和持久格式不变。

- 2026-09-17：搜索索引由搜索负责人持有独立工作范围；停止后拒绝新更新并等待已接受工作结束，调用方取消不遗弃索引写入，任务异常作为停止失败保留。暂停与恢复不向上层暴露内部任务，公开搜索入口和索引格式不变。

- 2026-09-17：空间网络恢复由自身负责人串行合并重复请求并持有重试；停止优先取消未开始循环，同时等待已开始的磁盘与网络收尾。调用方取消不遗弃工作，旧成功结果不会在停止后发布，失败保留原始来源；连接维护的公开入口不变。

- 2026-09-17：成员维护由自身负责人持有共享工作与停止边界；停止后拒绝新请求并等待已开始的维护与记录完成，单个调用方取消不终止共享维护，任务异常由负责人保留。暂停期间的提示合并到恢复后处理，不向调用方泄露内部步骤。

- 2026-09-17：空间会话提供唯一完整关闭动作，统一停止连接维护、成员维护与网络恢复并等待三者完成；任一失败不跳过其它收尾，全部失败合并保留，重复关闭结果稳定。上层不再拼接三个内部负责人的关闭步骤。

- 2026-09-17：Application 成为业务运行工作的完整生命周期负责人，统一暂停、恢复和关闭剪贴板、文件传输、搜索、历史维护与空间会话；调用方只执行一个动作，不编排内部步骤。关闭尝试全部负责人并合并失败，调用方取消不遗弃收尾，重复确认结果稳定；恢复出厂设置仅在完整停止成功后继续删除。

- 2026-09-17：宿主生命周期入口使用单一有序队列接收暂停、恢复和关闭；请求一旦接受便由内部任务持有，等待方离开或等待超时不撤销收尾。新业务动作只在运行状态接受，停止先通知已开始动作取消并继续等待真实资源释放，完成后才发布最终状态和关闭事件流。

- 2026-09-17：文件发送进度由独立负责人保存开始、进展与最终结果；网络恢复仅终止仍活动的发送，不重复覆盖已有最终状态。失败保留来源，公开事件不包含文件名或路径；传输身份和持久格式保持不变。

- 2026-09-17：生产会话统一持有空间安全状态、网络与 Application 会话及宿主剪贴板任务；暂停先封闭入口并通知动作取消，再等待磁盘与网络安全边界完成。恢复使用新的取消身份，旧会话延迟结果不能覆盖新状态；启动和交接失败完整回收已创建资源并保留原因。
...
- 2026-09-17：UniFFI 平台入口使用单一工作队列提交暂停、恢复、关闭和启动期间通知；平台回调只表达目标状态，重复目标合并，旧恢复不能越过新暂停或关闭。关闭等待工作与事件线程真实退出，等待方取消不遗弃已接受收尾；iOS 与 Android 共享同一实现。

- 2026-09-17：HarmonyOS 暂停将宿主预算传给 Engine，等待超时不宣称收尾完成；关闭改用完整等待入口，仅在资源真实释放后销毁运行期与观测装配。平台层不编排或推断内部会话步骤。

- 2026-09-17：移动验收宿主以真实剪贴板、文件访问和安全存储边界运行启动、暂停、恢复与关闭场景；iOS 与 Android 脚本只构建、安装并触发同一探针，不复制产品逻辑。实体设备执行属于设备验收，未运行时明确记为跳过。
