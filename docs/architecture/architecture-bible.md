...
- 2026-09-22：规格 034 开始最小多节点切片：test-only 有向成员网络只注册节点、施加分区、限制 frame 并路由 typed 成员历史消息，目标节点继续执行真实 Application endpoint；同时计划复用现有 connectivity host 和 network namespace 建立 nightly/手工单场景入口。该切片不实现完整配对，不代表五类路线覆盖完成，也不改变生产架构、公开接口、协议或持久格式。
