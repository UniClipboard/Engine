...
旧升级备份记录的保护材料属于同一恢复清单。已有恢复文件打开时，仍留在系统安全存储中的原值先合并进 userdata 密文并复读验证，随后才进入可重试清理；两处都缺失时保留升级安全门槛并返回稳定的不可重试结果。口令恢复后的完整运行期若启动或解锁失败，恢复状态进入 `Failed`，关闭同进程再次提交并明确 `restart_required`，由进程重启重新创建一次性宿主能力。
