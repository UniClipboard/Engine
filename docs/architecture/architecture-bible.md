...
当前设备列表只显示本机且本机成员有效时，可以修改加密口令，不要求处于升级重建或重新配对状态。该动作保留现有 MasterKey，只重新保护本机解锁材料并替换当前 Space control generation 绑定的配对认证资料；profile SQLite、blob、搜索和历史内容不得遍历或重加密。目标材料先写入由 Profile 密钥保护的恢复记录，再按同一目标前向完成并回读验证；启动时必须先完成遗留替换，不能在两套口令不一致时开放邀请。恢复记录不得包含明文口令，成功后立即删除。
...
单设备修改加密口令流程由 Application 的单一用例完整负责。产品先请求系统生成口令并展示保存提示，此时不修改安全资料；用户确认保存后再启用。两次动作都要求 Space 已解锁、本机成员有效且设备列表范围内不存在可用或暂停的其他成员；多设备、成员恢复中或成员资料不可确认时均拒绝。确认过程与邀请签发互斥，先撤销现有邀请，再调用 Infra 的完整替换能力。若当前仍有 `re_pairing_required` 提示，它只有在新设备真正加入后才清除，不因口令启用而清除。
...
单设备口令操作必须分别区分：Space 锁定、仍有其他成员、成员需要恢复、口令并非本轮系统生成、能力暂不可用和替换需要恢复。公开错误和诊断只能表达这些固定类别，不得包含口令或恢复材料；邀请签发遇到未完成的口令替换时必须先恢复同一目标，不能继续使用旧认证资料。
...
```text
Eligible -> Generated -> Confirmed -> Applying -> Active
              |             |
              +-> Discarded +-> Recovering -> Active
```

`Generated` 只表示口令已展示，旧口令仍然有效；只有用户确认保存后才进入替换。确认前退出可直接丢弃新口令。确认后的中断只能恢复并完成同一目标，不能回滚。任何阶段发现其他成员或成员状态不确定，都停止在修改安全资料之前。`Active` 表示新口令已生效，但重新配对提醒仍持续到新设备加入。
...
- iOS、Android 和 HarmonyOS 的绑定都提供“生成加密口令”和“确认修改加密口令”两步入口，只转换字符串和完成结果；资格检查、邀请撤销、替换顺序和恢复不进入绑定。口令输入、返回值和调试输出必须保持脱敏。
...
- 2026-09-14：设备列表只显示有效本机时新增两步修改加密口令流程，不要求处于升级重建状态。系统先生成并展示口令，用户确认保存后才撤销旧邀请并启用；任何仍显示其他设备或成员状态无法确认的 Space 均拒绝。能力及公开入口统一使用“修改加密口令”命名，不与重新配对状态混用。现有 MasterKey 与历史内容保持不变，受保护恢复记录保证中断后只向前完成；移动绑定公开相同动作，既有重新配对提醒仍由实际新设备加入结束。
