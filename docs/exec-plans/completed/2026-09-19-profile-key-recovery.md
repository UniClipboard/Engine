...
本规格是“资料密钥丢失恢复与旧设备迁移”的执行记录。状态：**Engine 实现与本地验收已完成；Desktop、实体平台和旧版本程序回退验收跳过**。
...
最新主线上的验证结果：密钥丢失宿主场景 9 项通过；完整宿主契约 19 项通过、1 项按设计跳过；Engine 单元测试 229 项通过、2 项按设计跳过；Infra 单元测试 976 项通过、8 项按设计跳过；Application 单元测试 949 项通过、1 项按设计跳过；公开契约 48 项、资料导入导出 2 项通过。0.19.1 样本和新建资料的 Desktop 恢复流程各 1 项通过，覆盖错误口令、正确口令、旧历史、新写入和再次启动。完整 workspace 编译、格式、Rust 规范、仓库结构和差异检查均通过。新 vault 测试确认原独立密钥名称和字节不会以明文出现。

Desktop 使用本地 Engine 覆盖完成 0.19.1 与新建资料两条恢复验收，未修改 Desktop 产品代码。实体系统钥匙串、iOS、Android、HarmonyOS、Windows 和旧版本程序回退记为跳过。

# 9. Acceptance Criteria

* [x] 完整安全材料清单与所有消费者闭合；本 profile 的正常系统钥匙串条目只有一个。
* [x] 新格式未新增明文业务负载，所有密钥和事务材料加密保存，明文探针通过。
* [x] 新安装与旧设备两条路径均实际通过；旧密钥原值保持，正文不因迁移批量重加密。
* [x] 已落盘的新副本通过口令独立验证后才删除旧条目，范围外条目保持不变。
* [ ] 所有文件写入断点的独立进程终止注入未执行；删除失败、重启续清理和无伪成功已通过。
* [x] 单个/整个钥匙串丢失时仍可调用公开口令恢复入口。
* [x] 完成迁移的 userdata 在空安全存储中恢复旧历史、新写入并重启。
...
* [x] 相关测试、完整编译、格式、Rust 风格、仓库结构与差异检查全部通过，已有基线失败单列。
