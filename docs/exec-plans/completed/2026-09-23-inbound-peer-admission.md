...
- **状态**：已完成（2026-09-25）；S1–S5a 与身份替换防护已实现并验证，收尾测试通过；生产路径订阅者检查跳过；S5b 恢复方式与身份文件消失原因转入[技术债](../tech-debt-tracker.md)（见“实施记录”）
- **日期**：2026-09-23
...
- 2026-09-25 收尾：身份替换防护提交为 `1a33211f`。全部切片后的收尾测试：`cargo nextest run -p uc-observability-contract -p uc-infra -p uc-application -p uc-engine --locked` 2603 项全部通过（以 nextest 执行，测试集合与 `cargo test` 相同）；`cargo test -p uc-infra --locked --test inbound_peer_rejection_diagnostics --test inbound_peer_single_owner -- --include-ignored` 1/1、4/4，无 ignored 项；A1–A5 中已无 `#[ignore]`。生产路径订阅者检查（本机双实例手动配对确认 Engine 文件中的 `peer.inbound.rejected`）未执行，记为跳过。S5b 恢复方式、身份文件消失原因登记为技术债；Desktop 恢复页的恢复出厂入口与宿主对 `local_identity_mismatch` 的解码兼容性由产品仓在发布前验证。准入判定此后由 [049](../active/049-single-owner-space-membership-rewrite.md) 的 `PeerAccess` 承接，S3 记录中的 `MembershipLedger` 指当时的实现。
