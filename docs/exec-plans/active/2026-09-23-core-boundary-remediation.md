...
| A4 | 网络准入判定写在 Infra | [`space/security/peer_admission.rs:32`](../../../crates/uc-infra/src/space/security/peer_admission.rs)：`local_join_active`、有 lineage、关系为 `Consistent` 才放行 | 已由[入站对端准入计划](2026-09-23-inbound-peer-admission.md)迁往 Application ledger。按新规范，判定式本身应由 Core 提供，Application 只负责读取；实施该计划时一并处理 |
