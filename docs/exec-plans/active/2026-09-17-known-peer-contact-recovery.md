...
- **职责**：完整的已认证业务交换成功后，从 Iroh 当前远端信息中提取 NodeId 与 relay，并更新现有地址记录。
- **输入**：已经完成身份、当前成员资格和业务协议验证的对端；当前远端地址快照。
- **输出**：只包含 NodeId 与 relay 的 `PeerAddressRecord`，或“没有稳定提示”。
- **关系**：写入失败不改变已经完成的业务结果；动态直连地址不写回；未知或未准入连接不写回。
...
- **Change**：构造 A、B、C 三设备：B 离线时 A/C 推进成员历史；A 换固定端口；B 保存 A 旧端口；阻断 B 的 mDNS 与公开发现；A 主动联系 B。
- **Risk**：网络故障必须有独立端口和防火墙计数证明，不能从 Engine 离线状态反推故障已经生效。

### Step 10

- **File**：同上及 `scripts/testing/run-connection-recovery-e2e.sh`
- **Change**：从 A 开始联系计时，20 秒内通过公开查询观察双方 Online；此前禁止 refresh、opportunity 和正文发送；上线后双向传输；默认重复三次并保留脱敏时间线。
...
Implementation: 事件发送位于身份映射之后、准入检查之前；Online 发布仍只在原准入确认成功之后。
```
...
* [ ] 稳定地址切片只保存 NodeId 与已验证 relay，不保存动态直连 IP/端口。
* [ ] 三设备验收能够独立证明旧端口失效、发现被阻断和 A 主动联系 B。
* [ ] 从 A 开始联系起 20 秒内双方 Online，且此前没有调用手动刷新、宿主机会通知或正文发送。
* [ ] Online 后双向正文传输成功；同一场景连续三次通过。
...
第一切片没有阻塞问题。后续地址切片开工前需要通过真实 Iroh 远端信息验证：成功的入站连接是否总能提供当前 relay，
以及 relay 变化时覆盖旧记录的准确条件。若库只能提供当前选中路径而不能证明 relay 已验证，则该切片只保存协议中由
对端签名或认证绑定的 relay，不从路径快照推断。
...
- [ ] Slice 2：稳定 relay 地址提示写回。
- [ ] Slice 3：三设备独立进程可重复验收。
...
- Windows 实机、真实公网、稳定 relay 回写和三设备独立进程场景尚未执行，均登记为跳过而非通过。
