...
会改变设备更新状态的队列写入包括：结清已失去资格的收件人、投递确认，以及投递失败后写入的退避
或拒绝。最初实现只在结清时通知；最后一项投递确认后若没有新的账本提交，宿主会一直停在“稍后
自动重试”。2026-09-26 起确认与退避写入同样通知，已由 Desktop 三节点端到端场景（离线成员
恢复后接受移除、组更新最后送达）验证宿主收敛。
