...
//!
//! 普通成员之间通过 `removal-exchange/1` 通道幂等同步已验证移除意图及其
//! 确认;已被移除的设备通过 `removal-late/1` 受限入口提交历史意图,通过
//! `removal-notice/1` 接收成员资格终止通知。受限入口只返回有界接收结果,
//! 不返回当前成员列表、在线状态、收敛摘要、安全代次、密钥或剪贴板状态;
//! 请求大小、单次数量、并发数与重试速率都有固定上限。恢复资料交接由
//! 受限恢复通道(`workspace-recovery/1`)承担。
