...
impl ProfileContentKeyVault {
    ...
    pub(crate) async fn resume(&self) -> Result<(), ProfileContentKeyVaultError> {
        self.run_owned(|owner| async move { owner.resume_resources().await })
            .await
    }

    async fn resume_resources(&self) -> Result<(), ProfileContentKeyVaultError> {
        let _io = self.io_lock.lock().await;
        {
        ...
        let lease = Arc::new(self.persistence.acquire_lease().await?);
