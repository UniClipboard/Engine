...
use uc_application::deps::MembershipProjectionPlan;

/// 成员读模型写入器：在调用方已开启的事务中按计划补齐、保留或清理成员、可信身份与地址。
///
/// 读模型只由成员记录的提交维护，与记录同一事务写入；无变化的行不重写。
pub(crate) struct MembershipProjectionWriter {
    cipher: RelationshipCipher,
}

impl<E: DbExecutor> EncryptedRelationshipStore<E> {
    pub(crate) async fn membership_projection_writer(
        &self,
    ) -> Result<MembershipProjectionWriter, RelationshipStoreError> {
        Ok(MembershipProjectionWriter {
            cipher: self.ready_cipher().await?,
        })
    }
}

impl MembershipProjectionWriter {
    pub(crate) fn apply(
        &self,
        ...
        let rows = encrypted_relationship::table
            .filter(encrypted_relationship::kind.eq_any(["member", "trusted_peer", "peer_address"]))
        ...
        for facts in &plan.members {
            let member_key = cipher.lookup_key(RelationshipKind::Member, facts.device_id.as_str());
            let member = existing
            ...
            if plan.trusted_device_ids.contains(&facts.device_id) {
                let key =
                    cipher.lookup_key(RelationshipKind::TrustedPeer, facts.device_id.as_str());
            ...
            if facts.device_id != plan.local_device_id {
                let key =
                    cipher.lookup_key(RelationshipKind::PeerAddress, facts.device_id.as_str());
                let previous = existing
        ...
        for (kind, identity, plaintext) in desired {
            ...
            let previous = existing.remove(&(kind.as_str().to_owned(), lookup_key.clone()));
            if previous
                .as_ref()
