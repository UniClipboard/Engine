...
use crate::operations::space::encryption_passphrase::{
    execute_confirm_encryption_passphrase_change, execute_generate_encryption_passphrase,
};
    ...
    async fn execute(
        ...
        let operation = async {
            match operation {
                ...
                Operation::GenerateEncryptionPassphrase => {
                    execute_generate_encryption_passphrase(self.current_facade().await?.as_ref())
                        .await
                }
                Operation::ConfirmEncryptionPassphraseChange(input) => {
                    execute_confirm_encryption_passphrase_change(
