...
async fn copied_clipboard_is_saved_and_written_remotely_in_one_trace() {
    clipboard_trace_case(false, Duration::ZERO).await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 6)]
#[ignore = "真实双设备写入失败验收，独占进程观测，精确运行"]
async fn clipboard_write_failure_keeps_saved_receipt_and_linked_error() {
    clipboard_trace_case(true, Duration::ZERO).await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 6)]
#[ignore = "真实双设备慢写入验收，独占进程观测，精确运行"]
async fn slow_clipboard_write_does_not_extend_network_receive() {
    clipboard_trace_case(false, Duration::from_millis(1500)).await;
}

async fn clipboard_trace_case(fail_write: bool, write_delay: Duration) {
    ...
    join_through(&source, &target, "Clipboard Target", &space_id).await;
    ...
    *source_clipboard.lock().expect("source clipboard") = HostClipboardSnapshot {
    ...
    let root = spans
        .iter()
        .find(|span| span.name == "clipboard.copy_and_sync")
    ...
    let dispatch = spans
        .iter()
        .find(|span| span.trace_id == root.trace_id && span.name == "clipboard_dispatch")
        .expect("dispatch");
