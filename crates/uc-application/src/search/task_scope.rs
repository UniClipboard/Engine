...
mod tests {
    ...
    async fn failures_survive_repeated_close_and_wait_for_other_work() {
        ...
        for _ in 0..2 {
            assert!(scope.spawn("search.test.panic", |_| async {
...
impl SearchTaskScope {
    ...
    {
        self.spawn_owned(name, work).is_some()
    }
    ...
    where
        ...
        Fut: Future<Output = ()> + Send + 'static,
    {
        if let Some(task) = self.spawn_owned(name, work) {
    ...
    fn spawn_owned<F, Fut>(&self, name: &'static str, work: F) -> Option<JoinHandle<()>>
    where
        F: FnOnce(CancellationToken) -> Fut + Send + 'static,
        Fut: Future<Output = ()> + Send + 'static,
    {
