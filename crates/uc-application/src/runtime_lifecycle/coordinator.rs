...
impl RuntimeLifecycleCoordinator {
    pub fn new(participants: RuntimeLifecycleParticipants) -> Self {
        Self {
            participants,
    ...
    async fn execute(
        ...
        if needs_cleanup || target == LifecycleTarget::Suspended {
            LifecycleError::from_errors(self.suspend(&context).await)?;
        ...
        state.phase = Phase::Incomplete;
        if let Err(primary) = self.resume(&context).await {
            let additional = self.suspend(&context).await;
