...
use super::{
    MembershipConvergence, MembershipConvergenceDeps, MembershipConvergenceError,
    MembershipConvergenceState, SharedDeviceRefreshDeviceState, SharedDeviceRefreshPhase,
...
struct BlockingSharedDeviceRefreshTransport {
    started: Arc<tokio::sync::Notify>,
    active: Arc<AtomicBool>,
}
...
#[async_trait]
impl MembershipGossipTransportPort for BlockingSharedDeviceRefreshTransport {
    async fn exchange(
        &self,
        _recipient: &DeviceId,
        message: MembershipGossipMessage,
...
async fn sponsor_preparation_persists_joiner_seed_for_existing_member() {
    ...
    let MembershipEvent::SponsorSeed(existing_member) = &joiner_delivery.batch().events[0] else {
    ...
    let MembershipEvent::SponsorSeed(joiner) = &existing_member_delivery.batch().events[0] else {
...
async fn sponsor_preparation_fails_when_joiner_delivery_cannot_be_queued() {
    ...
    assert!(matches!(
        error,
        MembershipConvergenceError::Outbox(MembershipOutboxRepositoryError::Repository(_))
    ));
}
...
#[tokio::test]
async fn shared_device_refresh_reuses_active_request_and_publishes_queryable_snapshot() {
    let members = Arc::new(InMemoryMemberRepository::default());
    let trusted = Arc::new(InMemoryTrustedPeerRepository::default());
    for (device_id, device_name, fingerprint_raw) in [
        ("device-a", "Device A", "AAAAAAAAAAAAAAAA"),
    ...
    trusted
        .save(&TrustedPeer {
            ...
            peer_fingerprint: fingerprint("BBBBBBBBBBBBBBBB"),
            trusted_at: chrono::DateTime::from_timestamp_millis(500).unwrap(),
        })
        .await
        .unwrap();
    let transport = Arc::new(BlockingMembershipTransport {
    ...
    let mut events = gossip.subscribe_shared_device_refresh();

    let first = gossip.start_shared_device_refresh().await.unwrap();
    let event = events.recv().await.unwrap();
    assert_eq!(event.request_id, first.request_id);
    assert_eq!(event.phase, SharedDeviceRefreshPhase::Started);
    let refresh_task = {
        let gossip = gossip.clone();
        let request_id = first.request_id.clone();
        tokio::spawn(async move {
            gossip
                .run_shared_device_refresh(request_id, SpaceId::from("space-a"))
                .await;
        })
    };
    transport.started.notified().await;

    let second = gossip.start_shared_device_refresh().await.unwrap();
    assert_eq!(second.request_id, first.request_id);
    let snapshot = gossip
        .shared_device_refresh_status(&first.request_id)
        .await
        .unwrap();
...
#[tokio::test]
async fn shared_device_refresh_connects_discovered_member_and_keeps_display_data() {
    let candidates = Arc::new(InMemoryCandidateRepository::default());
    let members = Arc::new(InMemoryMemberRepository::default());
    let trusted = Arc::new(InMemoryTrustedPeerRepository::default());
    let addresses = Arc::new(InMemoryPeerAddressRepository::default());
    ...
    trusted
        .save(&TrustedPeer {
            local_device_id: DeviceId::new("device-b"),
            peer_device_id: DeviceId::new("device-a"),
            peer_fingerprint: fingerprint("AAAAAAAAAAAAAAAA"),
            trusted_at: chrono::DateTime::from_timestamp_millis(500).unwrap(),
        })
        .await
        .unwrap();
    trusted
        .save(&TrustedPeer {
            local_device_id: DeviceId::new("device-b"),
            peer_device_id: DeviceId::new("device-d"),
            peer_fingerprint: fingerprint("DDDDDDDDDDDDDDDD"),
            trusted_at: chrono::DateTime::from_timestamp_millis(500).unwrap(),
        })
        .await
        .unwrap();
    let response = MembershipGossipMessage::SharedDevicePage(MembershipSharedDevicePage {
        space_id: SpaceId::from("space-a"),
    ...
    let duplicate_response =
        MembershipGossipMessage::SharedDevicePage(MembershipSharedDevicePage {
            ...
            seeds: vec![SponsorCandidateSeed {
                ...
                transport_address_blob: b"address-c".to_vec(),
                address_observed_at_ms: 700,
                source_device_id: DeviceId::new("device-d"),
                security_updates: Vec::new(),
                expires_at_ms: 10_000,
            }],
            next_after_device_id: None,
    ...
    let transport = Arc::new(ScriptedMembershipTransport {
        responses: Mutex::new(VecDeque::from([Ok(response), Ok(duplicate_response)])),
    ...
    let started = gossip.start_shared_device_refresh().await.unwrap();
    gossip
        .clone()
        .run_shared_device_refresh(started.request_id.clone(), SpaceId::from("space-a"))
        .await;
    let snapshot = gossip
        .shared_device_refresh_status(&started.request_id)
        .await
        .unwrap();

    assert_eq!(
        snapshot.devices,
        vec![super::SharedDeviceRefreshDevice {
            device_id: DeviceId::new("device-c"),
            device_name: "Device C".to_owned(),
...
#[tokio::test]
async fn shared_device_refresh_updates_the_same_request_after_offline_candidate_recovers() {
    let candidates = Arc::new(InMemoryCandidateRepository::default());
    let members = Arc::new(InMemoryMemberRepository::default());
    let trusted = Arc::new(InMemoryTrustedPeerRepository::default());
    let addresses = Arc::new(InMemoryPeerAddressRepository::default());
    ...
    let gossip = Arc::new(MembershipConvergence::new(MembershipConvergenceDeps {
        ...
        member_signatures: Arc::new(AcceptingMemberSignatures),
        fingerprint_factory: Arc::new(FixedFingerprintFactory),
        attestation: Arc::new(ScriptedAttestation(Mutex::new(VecDeque::from([
    ...
    let status = gossip
        ...
        .await
        .unwrap();
    assert_eq!(status.request_id, started.request_id);
    assert_eq!(
...
#[tokio::test]
async fn runtime_pause_and_shutdown_control_inflight_shared_device_refresh() {
    let started = Arc::new(tokio::sync::Notify::new());
    let active = Arc::new(AtomicBool::new(false));
    let members = Arc::new(InMemoryMemberRepository::default());
    for (device_id, device_name, fingerprint_raw) in [
    ...
    let refresh = gossip.start_shared_device_refresh().await.unwrap();
    tokio::time::timeout(Duration::from_secs(1), started.notified())
        .await
        .unwrap();
    assert!(active.load(Ordering::Acquire));
    ...
    assert_eq!(
        gossip
            .shared_device_refresh_status(&refresh.request_id)
            .await
            .unwrap()
            .request_id,
        refresh.request_id
