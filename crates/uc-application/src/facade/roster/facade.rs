...
impl MemberRosterFacade {
    ...
    pub async fn revoke_member(
        ...
        revocation = self.retry_group_delivery(revocation).await?;
        let removed = self
            .member_repo
            .remove(&device_id)
            .await
    ...
    fn revocation_view(revocation: GroupRevocationResult) -> MemberRevocationView {
        match revocation {
            ...
            GroupRevocationResult::Reliable {
                ...
                state: if status == RevocationStatus::Complete {
                    MemberRevocationState::Complete
                } else {
                    MemberRevocationState::Applied
                },
                pending_recipients,
            },
        }
