...
pub(crate) fn system_layer(_format: SystemLogFormat) -> Option<RuntimeLayer> {
    Some(Box::new(AppleSystemEvents {
...
pub(crate) fn system_layer(_format: SystemLogFormat) -> Option<RuntimeLayer> {
    Some(match tracing_android::layer("UcEngine") {
...
pub(crate) fn system_layer(format: SystemLogFormat) -> Option<RuntimeLayer> {
    match format {
        SystemLogFormat::Disabled => None,
        SystemLogFormat::Json => Some(Box::new(
        ...
        SystemLogFormat::HumanReadable => {
            Some(Box::new(tracing_subscriber::fmt::layer().with_ansi(false)))
        }
        SystemLogFormat::HumanReadableAnsi => {
            Some(Box::new(tracing_subscriber::fmt::layer().with_ansi(true)))
        }
    }
}
